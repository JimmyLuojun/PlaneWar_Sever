...
def upgrade():
    ...
    with op.batch_alter_table("scores", schema=None) as batch_op:
        ...
        batch_op.create_index(
            batch_op.f("ix_scores_user_id"), ["user_id"], unique=False
...
def downgrade():
    ...
    with op.batch_alter_table("scores", schema=None) as batch_op:
        ...
        # Drop indexes
        batch_op.drop_index(
            batch_op.f("ix_scores_user_id")
        )  # Added if created in upgrade
        batch_op.drop_index(batch_op.f("ix_scores_level"))
        # Make timestamp nullable again if it was changed
        batch_op.alter_column(
            "timestamp", existing_type=sa.DATETIME(), nullable=True
        )  # Assuming it was nullable before
